...
class CategoryListView(ListAPIView):
    ...
    def get_queryset(self):
        ...
        if user_id:
            is_users_category = Q(creator__clerk_id=user_id)
...
class PuzzleGuessResponse(APIView):
    ...
    def post(self, request, slug, level_num):
        ...
        for i in range(len(guessed_words)):
            if guessed_words[i] == real_words[i]:
                word_results.append(WordAccuracy.CORRECT)
            elif guessed_words[i] in real_words[i]:
        ...
        if (all_correct):
            ...
            score = round(PuzzleGuessResponse._similarityScore(message, puzzle.solution), 2)
            if score == 1:
                score = 0.99
    ...
    def get(self, request):
        ...
        if category_id:
            try:
                ...
                pass
        if after_position_raw is not None:
            try:
        ...
        annotated = (
            ...
            .values("id", "position", "par_score", "user_score", "user_attempts", "is_completed")[: BATCH_SIZE_DEFAULT + 1]
        )
        ...
        next_cursor = rows[-1]["position"] if rows else None
        payload = {
            "items": items,
            "next_cursor": next_cursor if has_more else -1,
            "has_more": has_more,
